...
public class Exercises {
	...
	public int[] rotateLeft3(int[] nums) {
		...
		for(int i = 0; i < nums.length -1; i++) {
			elementContainer = nums[i];
			nums[i] = nums[i + 1];
			nums[i + 1] = elementContainer;
		}
		return nums;
	}
	...
	/* not sure if preferred to keep array rather than create a new one
	 
	  int container;
	  for(int i = 0; i < nums.length / 2; i++){   
	  	container = nums[i];                       //stores old array number
	  	nums[i] = nums[nums.length - 1 - i];       //sets right index as new value from the right
	  	nums[nums.length - 1 - i] = container;     //sets the right index with the left index value (stored in the container)
	  	}
	  	return nums;
