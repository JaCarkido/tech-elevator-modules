...
public class HelloController {
	...
	public String displayGreeting(HttpServletRequest request) {
		...
		String name = request.getParameter("name");    				// get the value from name= on query string
		Greeter greeter = new Greeter(name);						// Instantiate a Greeter object using the value from query string
		request.setAttribute("greeting", greeter.getGreeting());	// add the vale from getGreeting() as an entry in the request map w/ the key greeting
